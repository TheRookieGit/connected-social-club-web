...
ALTER TABLE users ADD COLUMN IF NOT EXISTS has_kids VARCHAR(50);
...
-- 创建策略
CREATE POLICY "Users can manage their own education" ON user_education FOR ALL USING (auth.uid()::bigint = user_id);
CREATE POLICY "Users can manage their own work experience" ON user_work_experience FOR ALL USING (auth.uid()::bigint = user_id);
CREATE POLICY "Users can manage their own languages" ON user_languages FOR ALL USING (auth.uid()::bigint = user_id);
CREATE POLICY "Users can manage their own values" ON user_values FOR ALL USING (auth.uid()::bigint = user_id);
CREATE POLICY "Users can manage their own lifestyle" ON user_lifestyle FOR ALL USING (auth.uid()::bigint = user_id);
CREATE POLICY "Users can manage their own relationship preferences" ON user_relationship_preferences FOR ALL USING (auth.uid()::bigint = user_id);
