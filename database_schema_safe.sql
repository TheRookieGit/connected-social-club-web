...
BEGIN
    ...
    DROP POLICY IF EXISTS "Allow all operations" ON user_matches;
    DROP POLICY IF EXISTS "Allow all operations" ON user_messages;
    DROP POLICY IF EXISTS "Allow all operations" ON user_activity_logs;
    DROP POLICY IF EXISTS "Allow all operations" ON user_reports;
    DROP POLICY IF EXISTS "Allow all operations" ON user_subscriptions;
...
DROP TRIGGER IF EXISTS update_user_matches_updated_at ON user_matches;
...
-- 15. 为测试用户添加兴趣（使用 ON CONFLICT DO NOTHING）
